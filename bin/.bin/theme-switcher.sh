...
generate_codexbar_css() {
    ...
    _cb_recolor() { # src_name dest_name hex
        [ -f "$SRC/$1" ] && sed -E "s/#[Ff]{6}/$3/g; s/fill=\"white\"/fill=\"$3\"/g" "$SRC/$1" > "$OUT/$2"
    }
...
#custom-codexbar-grok   { color: $c_xai;       background-image: url("$OUT/grok.svg"); margin-right: 48px; }
...
#custom-codexbar-codex.stale,  #custom-codexbar-gemini.stale { opacity: 0.45; }
EOF
