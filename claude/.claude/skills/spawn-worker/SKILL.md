---
name: spawn-worker
description: "Use when spawning a background worker session, delegating to another agent, dispatching work, or using /spawn-worker."
---

# Spawn worker

Fire-and-forget spawner for background Codex sessions wired into the inter-session bus. The orchestrator returns immediately with the spawnee's short ID; progress arrives later as inter-session messages.
...
```
Bash("$HOME/.agents/skills/spawn-worker/scripts/spawn.sh [--worktree] <name> <task description>")
```
...
6. Runs `Codex --bg --name <name> "<prompt>"` and parses the short ID.
...
**Cleanup**: spawn.sh's stdout shows the `git worktree remove` command. Run it after the worker's PR merges. If `Codex --bg` itself fails post-worktree-creation, the script's ERR trap removes the orphaned worktree automatically.
...
| 5 | `Codex --bg` returned no short ID | read script stderr for raw output |
